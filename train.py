...
print(sys.path)
from read_tfrecord import read_single_tfrecord
import random
import cv2
...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    net = prefix.split('/')[-1]
    # label file
    label_file = os.path.join(base_dir, 'DATA/imglist.txt')
    ...
    dataset_dir = os.path.join(base_dir, 'DATA/tfrecord/RNet.tfrecord_shuffle')
    ...
    cls_loss_op, accuracy_op = net_factory(input_image, label, training=True)
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if (step + 1) % display == 0:
                ...
                cls_loss, lr, acc = sess.run(
                    [cls_loss_op, lr_op, accuracy_op],
                    feed_dict={input_image: image_batch_array, label: label_batch_array})
                total_loss = radio_cls_loss * cls_loss
                # landmark loss: %4f,
                print("%s : Step: %d/%d, accuracy: %3f, cls loss:"
                      " %4f, Total Loss: %4f ,lr:%f " % (
